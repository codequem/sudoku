...
ButtonOption startScreen::SudokuButtonStyle(Sudoku& sudo, int x, int y) {
	auto option = ButtonOption::Animated();
	option.transform = [&sudo, x, y, this](const EntryState& s) {
		auto element = text(s.label) | flex ;
		bool changeable = sudo.get_board_value(x, y) != 0;
		bool wrong = !sudo.get_checker()->get_result_board()[x][y];
		if (changeable) {
		...
		{
			element |= color(Color::RGB(187, 255, 0));
			//element |= color(Color::RGB(169, 219, 231));
		}
		if (s.focused)
			element |= bold;

		auto thecolor = color(Color::White);

		switch ((y / 3) * 3 + (x / 3))
		{
		case 1:
			thecolor = color(Color::RGB(222, 162, 162));
			break;
		case 2:
			thecolor = color(Color::RGB(196, 221, 161));
			break;
		case 3:
			thecolor = color(Color::RGB(161, 221, 220));
			break;
		case 4:
			thecolor = color(Color::RGB(161, 189, 221));
			break;
		case 5:
			thecolor = color(Color::RGB(175, 161, 221));
			break;
		case 6:
			thecolor = color(Color::RGB(207, 161, 221));
			break;
		case 7:
			thecolor = color(Color::RGB(155, 166, 11));
			break;
		case 8:
			thecolor = color(Color::RGB(221, 161, 184));
			break;
		case 0:
			thecolor = color(Color::RGB(221, 210, 161));
			break;
		}

		//将选中的数字有关的格子标注出来
		bool use_bgcolor = false;
		if (btn_col == y || btn_row == x) use_bgcolor = true;
		if (((btn_col / 3) * 3 + (btn_row / 3)) == ((y / 3) * 3 + (x / 3))) use_bgcolor = true;
		if (sudo.get_merge_board_value(btn_row, btn_col) != 0)
			if (sudo.get_merge_board_value(x,y) == sudo.get_merge_board_value(btn_row, btn_col)) use_bgcolor = true;
		if (btn_col == y && btn_row == x) use_bgcolor = false;

		return element | center | border | thecolor | flex | ((use_bgcolor) ? bgcolor(Color::RGB(62, 75, 74)) : nothing);
		};
	return option;
}
...
class ScreenManager {
public:
	static ScreenInteractive& getScreen() {
		static auto screen = ScreenInteractive::FitComponent();
		return screen;
	}
};
...
{
	auto btn_start_game = Button("进入数独游戏", [&] {startGame(UNKNOW_LEVEL);  }, Style());
	auto btn_rule_explan = Button("游戏规则说明", [&] {ruleExplan();  }, Style());
	auto btn_oper_explan = Button("游戏操作说明", [&] {operExplan();  }, Style());
	auto btn_exit_game = Button("退出游戏", [&] { exitGame(); }, Style());
	int row = 0;
	auto buttons = Container::Vertical({
		Container::Horizontal({btn_start_game},&row) | flex,
		Container::Horizontal({btn_rule_explan},&row) | flex,
		Container::Horizontal({btn_oper_explan},&row) | flex,
		Container::Horizontal({btn_exit_game},&row) | flex,
		});
	...
	screenManager.getScreen().Loop(main_component);
	return;
}

void startScreen::startGame(int level)
{
	if (level == UNKNOW_LEVEL)
		chooseLevel();
	else
	{
		std::string strings[81];
		...
		for (int row = 0; row < 9; ++row) {
			...
			for (int col = 0; col < 9; ++col) {
				sudoku.get_board()[row][col] == 0 ? strings[row * 9 + col] = " " : strings[row * 9 + col] = std::to_string(sudoku.get_board()[row][col]);
				auto thebgcolor = bgcolor(Color::Black);
				auto button = Button(&strings[row * 9 + col], [&, col, row] {
					...
					if (sudoku.get_board_value(row, col) == 0)
					{
						...
						prompt_text = "";
						//prompt_text = reinterpret_cast <const char*>( u8"\u2070\u00B9\u00B2");
	...
	sudoku_grid = CatchEvent(sudoku_grid, [&](Event event) {
			...
			else
			{
				...
				if (!use_guess)
				{
					...
					if (input_char >= "1" && input_char <= "9") {
						strings[index] = input_char;  // 更新字符串
						value = std::stoi(input_char);
					}
					else if (input_char == "0" || input_char == " ") {
						strings[index] = " ";  // 允许用户清空输入
						//strings[index] = reinterpret_cast <const char*>(u8"\u00B9\u2082\u00B3");
					}
					sudoku.one_step(btn_row, btn_col, value);
					std::ofstream out;
					out.open("steps.txt", std::ios::out | std::ios::app);
					out << "step:" << sudoku.get_step() << "Row:" << btn_row + 1 << " " << "Col:" << btn_col + 1 << std::endl;
					dynamic_cast<Checker*>(sudoku.get_checker())->print_result();
					dynamic_cast<Checker*>(sudoku.get_checker())->print_all_board();
				}
				else
				{
					int value = 0;
					...
					if (input_char >= "1" && input_char <= "9") {
						value = std::stoi(input_char);
						if (sudoku.get_guess_num(index) >= 3)
							sudoku.clear_guess(index);
						sudoku.add_guess(index, value);
						std::string temp_string = "";
						auto guess_board = sudoku.get_guess(index);
						for (int i = 0; i < sudoku.get_guess_num(index); i++)
						{
							if (i & 1)
		...
		auto undo_button = Button("撤销", [&] {
			...
			if (record.size() > 0 && sudoku.get_step() > 1) {
				auto last_record = record.at(sudoku.get_step() - 1);
				auto second_last_record = record.at(sudoku.get_step() - 2);
				...
				if (last_record.get_x() == second_last_record.get_x() && last_record.get_y() == second_last_record.get_y())
					strings[last_record.get_x() * 9 + last_record.get_y()] = std::to_string(second_last_record.get_value());
				else
					strings[last_record.get_x() * 9 + last_record.get_y()] = " ";
			}
				...
				for (int i = 0; i < 9; i++)
				{
					for (int j = 0; j < 9; j++)
					{
						if (sudoku.get_merge_board()[i][j] == 0)
							...
							strings[i * 9 + j] = std::to_string(sudoku.get_merge_board()[i][j]);
					}
				}
				});
		}) | color(Color::RGB(222, 162, 162));
		auto new_game_button = Button("再来一局", [&] { startGame(UNKNOW_LEVEL); }) | color(Color::RGB(196, 221, 161));
		auto back_button = Button("返回", [&] {
			//TODO 这里会导致游戏异常退出
			screenManager.getScreen().ExitLoopClosure()();
		...
		screenManager.getScreen().Loop(game_component);
	}
}

void startScreen::chooseLevel()
{
	ScreenManager screenManager;

	auto btn_easy = Button("简单", [&] {startGame(EASY_LEVEL); }, Style());
	auto btn_medium = Button("中等", [&] {startGame(MEDIUM_LEVEL); }, Style());
	auto btn_hard = Button("困难", [&] {startGame(HARD_LEVEL); }, Style());
	auto btn_return = Button("返回", [&] { screenManager.getScreen().ExitLoopClosure()(); }, Style());
	int row = 0;
	auto buttons = Container::Vertical({
	...
	screenManager.getScreen().Loop(level_component);
}

void startScreen::ruleExplan()
{
	ScreenManager screenManager;
	...
	rule_explan = CatchEvent(rule_explan, [&](Event event) {
		...
		if (event.is_mouse()) {
			if (event.mouse().button == Mouse::Right && event.mouse().motion == Mouse::Pressed) {
				screenManager.getScreen().ExitLoopClosure()();//退出当前循环，回到上一层循环，即主界面
				return true;
			}
		}
		});

	screenManager.getScreen().Loop(rule_explan);
}

void startScreen::operExplan()
{
	ScreenManager screenManager;
	...
	auto open_explan = Renderer([&] {
		return vbox({
			...
			text("                ") | hcenter,
			text("                ") | hcenter,
			text("如果你填写的数字不符合数独游戏的规则，会有提示。") | hcenter | color(Color::RGB(184, 233, 224)),
			text("                ") | hcenter,
			text("                ") | hcenter,
			text("                ") | hcenter,
			text("祝您游戏愉快！") | hcenter | color(Color::RGB(214, 140, 241)),
	...
	screenManager.getScreen().Loop(open_explan);
}
...
void startScreen::check(Sudoku& sudo)
{
	ScreenManager screenManager;
		...
		auto text_component = Renderer([&] {
			...
			auto button_main_menu = Button("返回主菜单", [&] {
				...
				auto button_retry = Button("再来一局", [&] {
					...
					screenManager.getScreen().Loop(result_component);
	}
	else
	{
		auto text_component = Renderer([&] {
